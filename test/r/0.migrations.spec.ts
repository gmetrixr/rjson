...
import { r, migrations, en, RT, RecordNode, rtp, createRecord, fn } from "../../src";
...
import { initialMigrateProjectRJson, migrationsForNewProject } from "../../src/migrations/project";
...
describe("Tests for file source based migrations", () => {

  let project: RecordNode<RT.project>;
  beforeEach(() => {
    project = createRecord(RT.project);
    migrationsForNewProject(project);
    const projectF = r.project(project);
    // for testing purposes
    projectF.set(rtp.project.version, 100);
    const initialSceneId = projectF.getInitialSceneId();
    const element = projectF.addElementOfTypeToScene({
      sceneId: initialSceneId,
      elementType: en.ElementType.popup,
    });

    if(element) {
      const elementF = r.element(element);
      const item = elementF.addBlankRecord(RT.item);
      if(item) {
        const itemF = r.record(item);
        itemF.set(rtp.item.item_source, { id: 1, uri: "some random uri which is not used" });
        itemF.set(rtp.item.item_source_type, "media");
        item.id = 1;
      }
   ...
   * 3. file injection
   */
  it("test correct migration before file injection - INCORRECT FLOW", () => {
    initialMigrateProjectRJson(project);
    migrateProjectRJson(project);
    const projectF = r.project(project);
    projectF.injectSourceIntoProject(sourceMap);

    const item = projectF.getAllDeepChildrenWithFilter(RT.item, e => e.id === 1)[0];
    expect(item.props.item_source_type).to.not.eq("image");
  });
   ...
   * 3. normal migrations
   */
  it("test correct migration before file injection - CORRECT FLOW", () => {
    initialMigrateProjectRJson(project);
    const projectF = r.project(project);
    projectF.injectSourceIntoProject(sourceMap);
    migrateProjectRJson(project);
    ...
    const item = projectF.getAllDeepChildrenWithFilter(RT.item, e => e.id === 1)[0];
    expect(item.props.item_source_type).to.eq("image");
  });
});